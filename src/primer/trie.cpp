...
auto Trie::Get(std::string_view key) const -> const T * {
  ...
  for (const char i : key) {
    auto itr = cnt_node->children_.find(i);
    if (itr == cnt_node->children_.end()) {
      return nullptr;
    }
    cnt_node = cnt_node->children_.at(i).get();
  }
  if (cnt_node == nullptr) {
    return nullptr;
  }
  if (!cnt_node->is_value_node_) {
    return nullptr;
  }
  const auto cnt_value_node = dynamic_cast<const TrieNodeWithValue<T> *>(cnt_node);
  if (cnt_value_node == nullptr) {
    return nullptr;
  }
...
auto Trie::Put(std::string_view key, T value) const -> Trie {
  ...
  for (char i : key) {
    ...
    if (cnt_node->children_.find(i) == cnt_node->children_.end()) {
      cnt_node = nullptr;
    } else {
      cnt_node = cnt_node->children_.at(i);
    }
  }
  ...
  if (cnt_node == nullptr) {
    leaf_node = std::make_shared<const TrieNodeWithValue<T>>(std::move(shared_value));
  } else {
    leaf_node = std::make_shared<const TrieNodeWithValue<T>>(cnt_node->children_, std::move(shared_value));
  }
  std::shared_ptr<const TrieNode> child_node = leaf_node;
  for (int i = static_cast<int>(key.size()) - 1; i >= static_cast<int>(node_stack.size()); i--) {
    std::map<char, std::shared_ptr<const TrieNode>> children_map{{key[i], child_node}};
    std::shared_ptr<const TrieNode> temp_node = std::make_shared<const TrieNode>(children_map);
  ...
  for (int i = node_stack.size() - 1; i >= 0 && !node_stack.empty(); i--) {
    ...
    const_cast<TrieNode *>(temp_node_clone.get())->children_[key[i]] = child_node;
    child_node = static_cast<std::shared_ptr<const TrieNode>>(temp_node_clone);
  }
...
auto Trie::Remove(std::string_view key) const -> Trie {
  ...
  for (char i : key) {
    ...
    auto it = cnt_node->children_.find(i);
  ...
  for (int i = static_cast<int>(key.size()) - 1; i >= 0; i--) {
    ...
    node_stk.pop();
    if (!flag && cnt_node->children_.size() == 1 && !cnt_node->is_value_node_) {
      child_node = nullptr;
      cnt_node = nullptr;
      continue;
    }
    ...
    if (child_node != nullptr) {
      const_cast<TrieNode *>(cnt_node.get())->children_[key[i]] = child_node;
    }
    child_node = cnt_node;
  }
  return Trie(child_node);
